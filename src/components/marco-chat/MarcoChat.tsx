...
const SOUVENIR_VALUE = "__souvenir";
...
export function MarcoChat({ open, onClose, tours, config, locale }: MarcoChatProps) {
  ...
  /** Leads the souvenir shelf with the tour the guest was last shown. */
  const lastTourIdRef = useRef<string | undefined>(undefined);
  ...
  const recommendTours = useCallback(
    async (finalAnswers: MarcoAnswers) => {
      const recommendation = recommend(finalAnswers);
      const picked = recommendation.tourIds
        .map((id) => findTour(id))
      ...
      for (const tour of picked) {
        setMessages((prev) => [
          ...prev,
          { id: nextId(), kind: "tour", tour, recommendation },
        ]);
        await delay(220);
        if (!mountedRef.current) return;
      }

      lastTourIdRef.current = picked[0]?.id;

      await botSay(copy.tapInstruction(config.operatorName));
      if (!mountedRef.current) return;
      setStep("recommend");
      setQuickReplies([
        { label: getSouvenirCopy(locale).quickReply, value: SOUVENIR_VALUE },
        { label: copy.startOver, value: RESTART_VALUE },
      ]);
    },
    [botSay, config.operatorName, copy, findTour, locale, nextId],
  );
  ...
  const handleSelect = useCallback(
    async (reply: QuickReply) => {
      ...
      if (reply.value === SOUVENIR_VALUE) {
        pushUser(reply.label);
        setMessages((prev) => [
          ...prev,
          { id: nextId(), kind: "souvenir", tourId: lastTourIdRef.current },
        ]);
        return;
      }
