...
import { OPEN_PLAN_TRIP_EVENT } from "./HomeIntentTracks";
...
export function HomeHeader({
  ...
  // Deep link: open the concierge chat automatically when the URL carries
  // ?chat=1 (or ?plan=1). Lets video/social CTAs like "Start planning" land
  // guests straight in the chat. Read client-side to avoid a static-render
  // Suspense bailout from useSearchParams.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("chat") === "1" || params.get("plan") === "1") {
      setIsPlanTripOpen(true);
    }
  }, []);

  // Let the homepage intent-trio ("Plan my trip" card) open the concierge
  // without duplicating the modal state.
  useEffect(() => {
    const open = () => setIsPlanTripOpen(true);
    window.addEventListener(OPEN_PLAN_TRIP_EVENT, open);
    return () => window.removeEventListener(OPEN_PLAN_TRIP_EVENT, open);
  }, []);
