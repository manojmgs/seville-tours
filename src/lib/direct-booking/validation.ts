...
export function parseDirectBooking(raw: unknown): ValidationResult {
  ...
  return {
    ...
    value: {
      ...
      // Server trust boundary: this is only a browser-reported OBSERVATION of a
      // read-only verification. NEVER treat it as redemption authorization or as
      // proof of remaining balance — the commit flow must re-verify server-side.
      // Coerced to a strict boolean; any non-`true` value is dropped.
      giftVerificationObserved: body.giftVerificationObserved === true ? true : undefined,
