...
export function buildOperatorEmail(request: DirectBookingRequest): OperatorEmail {
  ...
  // Full gift code REQUIRED for MANUAL operator redemption in the ParaUsted
  // dashboard (no automated commit yet). This email body must be transmitted
  // securely and never logged (see ConsoleEmailSender / GDPR note above).
