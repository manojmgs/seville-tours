...
export type DirectBookingInput = {
  ...
  giftCode?: string;
  /**
   * Advisory-only signal that the browser OBSERVED a read-only verification result.
   * It is NOT authorization to redeem and NOT proof that balance still remains — the
    * authenticated ParaUsted dashboard must re-verify before Carlos commits there.
   */
  giftVerificationObserved?: boolean;
  paymentProof?: string;
  notes?: string;
  locale: string;
};
