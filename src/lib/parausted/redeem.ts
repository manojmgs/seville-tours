...
/** Checks the loose client-safe shape before contacting ParaUsted. */
export function isGiftCardCodeFormatValid(rawCode: string): boolean {
  return CODE_PATTERN.test(normalizeGiftCardCode(rawCode));
}
