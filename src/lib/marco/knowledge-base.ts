...
const INTENTS: Intent[] = [
  {
    // Checked first so "how much walking" can't be stolen by the price intent's
    // broad "how much" keyword below.
    keywords: [
      // en
      "walk", "walking",
      // es
      "caminar", "caminata", "se camina",
      // fr
      "marche", "marcher", "à pied",
      // ar
      "مشي", "المشي",
    ],
    respond: ({ config, copy }) => ({
      kind: "escalate",
      text: copy.walkingAnswer(config.operatorName),
    }),
  },
  {
    // Checked before "booking" so "cancel my booking" isn't stolen by that
    // intent's broad "booking" keyword below.
    keywords: [
      // en
  ...
  },
  {
    ...
    respond: ({ config, copy }) => ({
      kind: "reply",
      text: copy.privateAnswer(config.operatorName),
    }),
  },
  {
    keywords: [
