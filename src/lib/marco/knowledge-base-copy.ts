...
export type KbCopy = {
  ...
  privateAnswer: (operator: string) => string;
  walkingAnswer: (operator: string) => string;
...
const en: KbCopy = {
  ...
  privateAnswer: (operator) =>
    `We offer both small-group and private experiences — the exact format depends on the tour. Some tours run as small groups; others can be arranged privately. For a custom private arrangement, ${operator} confirms the details before anything is booked.`,
  walkingAnswer: (operator) =>
    `Walking level varies by tour and route, so I don't want to guess. Let me connect you with ${operator} — he'll confirm what's realistic for your pace.`,
...
const es: KbCopy = {
  ...
  privateAnswer: (operator) =>
    `Ofrecemos tanto tours en grupo reducido como experiencias privadas — el formato exacto depende del tour. Algunos tours son en grupo reducido; otros se pueden organizar de forma privada. Para un arreglo privado a medida, ${operator} confirma los detalles antes de reservar nada.`,
  walkingAnswer: (operator) =>
    `El nivel de caminata varía según el tour y la ruta, así que prefiero no adivinar. Deja que te conecte con ${operator} — él te confirmará lo que es realista para tu ritmo.`,
  languageAnswer: (operator) =>
...
const fr: KbCopy = {
  ...
  privateAnswer: (operator) =>
    `Nous proposons à la fois des visites en petit groupe et des expériences privées — le format exact dépend de la visite. Certaines visites se font en petit groupe ; d'autres peuvent être organisées en privé. Pour un arrangement privé sur mesure, ${operator} confirme les détails avant toute réservation.`,
  walkingAnswer: (operator) =>
    `Le niveau de marche varie selon la visite et l'itinéraire, donc je préfère ne pas deviner. Laissez-moi vous mettre en relation avec ${operator} — il confirmera ce qui est réaliste pour votre rythme.`,
  languageAnswer: (operator) =>
...
const ar: KbCopy = {
  ...
  privateAnswer: (operator) =>
    `نقدّم جولات في مجموعات صغيرة وتجارب خاصة أيضًا — يعتمد الشكل الدقيق على الجولة نفسها. بعض الجولات تكون في مجموعة صغيرة، وأخرى يمكن ترتيبها بشكل خاص. للترتيب الخاص المخصص، يؤكد ${operator} التفاصيل قبل أي حجز.`,
  walkingAnswer: (operator) =>
    `يختلف مستوى المشي حسب الجولة والمسار، لذا أفضّل ألا أخمّن. دعني أوصلك بـ ${operator} — سيؤكد لك ما هو مناسب لوتيرتك.`,
  languageAnswer: (operator) =>
